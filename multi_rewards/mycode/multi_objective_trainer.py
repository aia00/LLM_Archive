"""Dynamic-weight GRPO trainer for multi-objective RLHF (Method C)."""
...
class GradientWeightedGRPOTrainer(DynamicGRPOTrainer):
    """GRPO trainer with gradient-based dynamic reward weights."""

    def __init__(
        self,
        *args,
        dynamic_eta: float = 0.5,
        dynamic_mu: float = 1.0,
        **kwargs,
    ):
    ...
    def _compute_loss(self, model, inputs):
        ...
        grads = []
        for idx, loss_i in enumerate(loss_list):
        ...
        influences = []
        for grad_i in grads:
            dot = torch.tensor(0.0, device=completion_mask.device)
        ...
        weights = self.reward_weights.to(influence_vec.device)
        weights = weights * torch.exp(self.dynamic_eta * self.dynamic_mu * influence_vec)
        weights = weights / weights.sum().clamp(min=1e-8)
        self.reward_weights = weights.detach()

        weighted_rewards = (rewards_per_func * weights.unsqueeze(0)).sum(dim=1)
        weighted_advantages = self._compute_group_advantages(
            weighted_rewards, group_size
        )

        loss = self._compute_loss_from_advantages(
            inputs, weighted_advantages, completion_mask, loss_terms
        )
        return loss
