...
class ScriptArguments:
    ...
    model_name_or_path: str = field(
        metadata={"help": "Model name or path to the model to evaluate."}
    )
    ...
    num_examples: int | None = field(
        default=None, metadata={"help": "Number of examples to evaluate."}
    )
...
if __name__ == "__main__":
    ...
    )  # very generous max token length
    ...
    if "gpt" in script_args.judge_model:
        ...
        judge.client = OpenAI(base_url="https://api.apiyi.com/v1")
    else:
        judge = HfPairwiseJudge(script_args.judge_model)

    completions = [
        [c0, c1]
        for c0, c1 in zip(reference_completions, model_completions, strict=True)
    ]
