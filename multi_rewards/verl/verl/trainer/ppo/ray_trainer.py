...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.multi_objective_mode = None
        self.multi_objective_weights = None
        self.dynamic_eta = None
        ...
        reward_kwargs = reward_cfg.get("reward_kwargs", {}) if hasattr(reward_cfg, "get") else {}
        self.multi_objective_mode = reward_kwargs.get("multi_objective_mode")
        ...
        if self.multi_objective_mode == "hypervolume":
            try:
    ...
    def _resolve_multi_objective_weights(self):
    ...
    def _build_token_level_scores(
        reward_values: torch.Tensor, response_mask: torch.Tensor
    ) -> torch.Tensor:
        scores = torch.zeros_like(response_mask, dtype=reward_values.dtype)
        lengths = response_mask.sum(dim=-1).long().clamp(min=1)
        idx = torch.arange(scores.size(0), device=scores.device)
        scores[idx, lengths - 1] = reward_values
        return scores
    ...
    def _update_pareto_meta_scale(self, point):
        if self._pareto_buffer is None:
            return None
        delta_hv = self._pareto_buffer.hypervolume_with(point) - self._pareto_buffer.hypervolume()
        self._pareto_buffer.add(point)
        meta_scale = 0.5 + 1.5 * math.tanh(delta_hv)
        for reward_fn in (self.reward_fn, self.val_reward_fn):
            reward_kwargs = getattr(reward_fn, "reward_kwargs", None)
            if isinstance(reward_kwargs, dict):
                reward_kwargs["meta_scale"] = meta_scale
        return {"pareto/meta_scale": meta_scale, "pareto/hv_delta": delta_hv}
    ...
    def _validate(self, merged: bool = False):
        ...
        self._last_val_pareto_point = self._compute_pareto_point(reward_extra_infos_dict)
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            if self.multi_objective_mode == "hypervolume" and self._last_val_pareto_point is not None:
                meta_metrics = self._update_pareto_meta_scale(self._last_val_pareto_point)
                if meta_metrics:
                    val_metrics.update(meta_metrics)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.multi_objective_mode == "dynamic" and reward_extra_infos_dict:
                            resolved_weights = self._resolve_multi_objective_weights()
                            if resolved_weights is not None:
                                batch.meta_info["dynamic_init_weights"] = list(resolved_weights)
                            if self.dynamic_eta is not None:
                                batch.meta_info["dynamic_eta"] = self.dynamic_eta
                            if self.dynamic_mu is not None:
                            ...
                            for key in ("reward_acc", "reward_conc", "reward_clar"):
                                ...
                                reward_values = torch.tensor(
                                    values, dtype=reward_tensor.dtype, device=reward_tensor.device
                                )
                                reward_components[key] = self._build_token_level_scores(
                            ...
                            if len(reward_components) == 3:
                                ...
                                for key in ("reward_acc", "reward_conc", "reward_clar"):
                                    ...
                                    advantages_components.append(adv)
