...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(self, config: ActorConfig, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        self.dynamic_reward_weights = None
    ...
    def update_policy(self, data: DataProto):
        ...
        if "advantages_components" in data.batch.keys():
            select_keys.append("advantages_components")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    advantages_components = model_inputs.get("advantages_components")
                    ...
                    if use_dynamic_weights and advantages_components is not None:
                        if self.dynamic_reward_weights is None:
                            self.dynamic_reward_weights = torch.full(
                                (advantages_components.size(-1),),
                                1.0 / advantages_components.size(-1),
                                device=log_prob.device,
                            )

                        per_obj_losses = []
                        for i in range(advantages_components.size(-1)):
                            pg_loss_i, _ = policy_loss_fn(
                                old_log_prob=old_log_prob,
                                log_prob=log_prob,
                                advantages=advantages_components[..., i],
                                response_mask=response_mask,
                                loss_agg_mode=loss_agg_mode,
                                config=self.config,
                                rollout_is_weights=rollout_is_weights,
                            )
                            per_obj_losses.append(pg_loss_i)

                        params = [p for p in self.actor_module.parameters() if p.requires_grad]
                        grads = []
                        for loss_i in per_obj_losses:
                        ...
                        for param_idx in range(len(params)):
                            ...
                            sum_grads.append(accum)

                        influences = []
                        for grad_i in grads:
                            dot = torch.tensor(0.0, device=log_prob.device)
                            for g_i, g_sum in zip(grad_i, sum_grads):
                                if g_i is None or g_sum is None:
                                    continue
                                dot = dot + (g_i * g_sum).sum()
                            influences.append(dot)

                        influence_vec = torch.stack(influences).detach()
                        weights = self.dynamic_reward_weights.to(log_prob.device)
                        weights = weights * torch.exp(float(dynamic_eta) * float(dynamic_mu) * influence_vec)
